...
const KidsTraditionalPage = ({ onNavigate }) => {
  const { searchQuery } = useShop();
  const [selectedSubcategory, setSelectedSubcategory] = useState('All');
  const [sortBy, setSortBy] = useState('popular');

  const subcategories = [
    { id: 'All', label: 'All Kids Traditional' },
    { id: 'Agbada', label: '👑 Mini Agbada Sets (Boys)' },
    { id: 'Ankara', label: '✨ Kids Ankara Dresses (Girls)' }
  ];

  const kidsTraditionalProducts = ALL_COLLECTION.filter((p) => {
    return p.category === 'Kids' && p.isTraditional === true;
  });

  const filtered = kidsTraditionalProducts.filter((p) => {
      ...
      <div className="bg-zinc-950 text-white py-12 sm:py-16 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex items-center gap-2 text-xs text-amber-400 font-semibold uppercase tracking-wider mb-3">
            ...
            <span className="text-white">Kids Traditional Collection</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 bg-amber-400/10 border border-amber-400/30 px-3 py-1 rounded-full text-amber-300 text-xs font-bold uppercase tracking-widest mb-3">
                ...
                <span>Miniature Royalty for Family Celebrations</span>
              </div>
              <h1 className="text-3xl sm:text-5xl font-black font-serif uppercase tracking-tight text-white">
                Kids Traditional Nigerian Wear
              </h1>
              <p className="text-zinc-300 text-sm sm:text-base max-w-2xl mt-2 font-light">
                Handcrafted miniature Grand Agbada sets with matching caps (Fila) and vibrant Dutch Wax Ankara dresses.
              </p>
            </div>

            <button
              onClick={() => onNavigate('kids-standard')}
            ...
            >
              <span>View Standard Kids (Jeans & Tees) →</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">
            Showing <strong className="text-zinc-900">{sorted.length}</strong> miniature traditional pieces
          </p>
          <div className="flex items-center gap-2 text-xs text-amber-700 font-semibold">
            <Scissors className="w-3.5 h-3.5" />
            <span>Soft Anti-Irritation Cotton Linings</span>
          </div>
        </div>
        ...
        {sorted.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-zinc-300">
            <p className="text-base font-semibold text-zinc-700">No traditional kids garments found matching your filter.</p>
            <button
...
export default KidsTraditionalPage;
