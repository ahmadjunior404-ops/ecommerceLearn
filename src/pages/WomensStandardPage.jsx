...
const WomensStandardPage = ({ onNavigate }) => {
  ...
  const subcategories = [
    { id: 'All', label: 'All Women\'s Standard' },
    { id: 'Jeans', label: '👖 Wide-Leg & Denim' },
    { id: 'Tops', label: '👚 Linen Tops & Blouses' },
    { id: 'Footwear', label: '👡 Summer Leather Sandals' },
    { id: 'Loungewear', label: '✨ Knit Lounge Pants' }
  ];

  // Filter for standard modern women's clothing
  const womensStandardProducts = ALL_COLLECTION.filter((p) => {
    return p.category === 'Women' && p.isTraditional === false;
  });

  const filtered = womensStandardProducts.filter((p) => {
      ...
      <div className="bg-zinc-950 text-white py-12 sm:py-16 relative overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img
            ...
            alt="Standard Contemporary Wear"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-xs text-amber-400 font-semibold uppercase tracking-wider mb-3">
            ...
            <span className="text-white">Women's Standard Collection</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 bg-amber-400/10 border border-amber-400/30 px-3 py-1 rounded-full text-amber-300 text-xs font-bold uppercase tracking-widest mb-3">
                ...
                <span>Modern Everyday Chic & Summer Fits</span>
              </div>
              <h1 className="text-3xl sm:text-5xl font-black font-serif uppercase tracking-tight text-white">
                Standard Women's Clothes
              </h1>
              <p className="text-zinc-300 text-sm sm:text-base max-w-2xl mt-2 font-light">
                High-rise wide leg denim, breathable pure linen wrap blouses, artisan summer leather sandals, and cozy modal lounge pants.
              </p>
            </div>

            {/* Switch to Traditional Women's */}
            <button
              onClick={() => onNavigate('womens-traditional')}
            ...
            >
              ...
              <span>View Women's Ankara & Aso-Oke Page →</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">
            Showing <strong className="text-zinc-900">{sorted.length}</strong> women's modern essentials
          </p>
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            <span className="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
            <span>All Items In Stock & Ready to Ship</span>
          </div>
        </div>
        ...
        {sorted.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-zinc-300">
            <p className="text-base font-semibold text-zinc-700">No women's standard garments match your filters.</p>
            <button
...
export default WomensStandardPage;
