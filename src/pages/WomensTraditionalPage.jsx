...
import { ArrowLeft, Sparkles, Crown, Scissors } from 'lucide-react';

const WomensTraditionalPage = ({ onNavigate }) => {
  const { searchQuery } = useShop();
  const [selectedSubcategory, setSelectedSubcategory] = useState('All');
  const [sortBy, setSortBy] = useState('popular');

  const subcategories = [
    { id: 'All', label: 'All Women\'s Traditional' },
    { id: 'Ankara', label: '✨ Dutch Wax Ankara Gowns' },
    { id: 'Aso-Oke', label: '🧵 Aso-Oke Couture Sets' },
    { id: 'Adire', label: '🎨 Hand-Dyed Silk Adire' },
    { id: 'Boubou', label: '👑 Beaded Boubous & Kaftans' }
  ];

  const womensTraditionalProducts = ALL_COLLECTION.filter((p) => {
    return p.category === 'Women' && p.isTraditional === true;
  });

  const filtered = womensTraditionalProducts.filter((p) => {
      ...
      <div className="bg-zinc-950 text-white py-12 sm:py-16 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-xs text-amber-400 font-semibold uppercase tracking-wider mb-3">
            ...
            <span className="text-white">Women's Traditional Couture</span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 bg-amber-400/10 border border-amber-400/30 px-3 py-1 rounded-full text-amber-300 text-xs font-bold uppercase tracking-widest mb-3">
                ...
                <span>Ceremonial & Luxury Nigerian Womenswear</span>
              </div>
              ...
              <p className="text-zinc-300 text-sm sm:text-base max-w-2xl mt-2 font-light">
                Exquisite Dutch Wax Ankara maxi dresses, handwoven Aso-Oke corseted sets, Abeokuta silk Adire ensembles, and beaded Boubous.
              </p>
            </div>

            <button
              onClick={() => onNavigate('womens-standard')}
            ...
            >
              <span>View Standard Women's (Jeans & Tops) →</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">
            Showing <strong className="text-zinc-900">{sorted.length}</strong> handcrafted traditional gowns & couture sets
          </p>
          <div className="flex items-center gap-2 text-xs text-amber-700 font-semibold">
            <Scissors className="w-3.5 h-3.5" />
            <span>Custom Tailoring & Aso-Ebi Available</span>
          </div>
        </div>
        ...
        {sorted.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-zinc-300">
            <p className="text-base font-semibold text-zinc-700">No traditional women's garments found matching your filter.</p>
            <button
...
export default WomensTraditionalPage;
